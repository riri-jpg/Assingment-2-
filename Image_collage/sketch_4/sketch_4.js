 ...
 *
 * TITLE : Transparency
 * DESCRIPTION : EACH IMAGES HAVE VARYING TRANSPARENCY WHEN CLICKED AND CHNAGES POSITONS 
 * 
 */
...
function setup() {
  ...
  imageMode(CENTER);
  noLoop(); // static — click to regenerate
...
// THE EFFECT: each image gets a random transparency when drawn
function drawCollageItems(items) {
  for (let i = 0; i < items.length; i++) {
    let item  = items[i];
    let alpha = random(26, 255); // random transparency, never below ~10%
    tint(255, 255, 255, alpha);  // white tint + random alpha
    push();
  ...
  noTint(); // reset after all items drawn
}
...
// Click anywhere to regenerate a new random collage
